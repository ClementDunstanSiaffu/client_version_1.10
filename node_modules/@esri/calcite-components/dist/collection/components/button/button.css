...
.calcite-button--loader calcite-loader {
  margin: 0px;
  transition: width var(--calcite-internal-animation-timing-slow) ease-in-out, opacity var(--calcite-internal-animation-timing-slow) ease-in-out, transform var(--calcite-internal-animation-timing-slow) ease-in-out;
}
...
:host([appearance=transparent][color=neutral].cancel-editing-button) button {
  ...
  border-top-color: var(--calcite-ui-border-input);
