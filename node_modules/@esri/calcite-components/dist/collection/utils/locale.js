...
export function createLocaleNumberFormatter(locale, numberingSystem = defaultNumberingSystem) {
...
export function getMinusSign(locale) {
  const formatter = createLocaleNumberFormatter(locale);
  const parts = formatter.formatToParts(-9);
  return parts.find((part) => part.type === "minusSign").value;
}
export function localizeNumberString(numberString, locale, displayGroupSeparator = false, numberingSystem) {
  return sanitizeExponentialNumberString(numberString, (nonExpoNumString) => {
    if (nonExpoNumString) {
