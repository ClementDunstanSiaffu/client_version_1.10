...
function createOrgNotification(options) {
    var url = get_portal_url_1.getPortalUrl(options) + "/portals/self/createNotification";
...
function _generateBatchRequests(options) {
    var userBatches = array_1.chunk(options.users, options.batchSize || 25);
...
function _generateRequestOptions(users, baseOptions) {
    ...
    requestOptions.params = tslib_1.__assign(tslib_1.__assign({}, requestOptions.params), { users: users, subject: baseOptions.subject, message: baseOptions.message, notificationChannelType: requestOptions.notificationChannelType });
